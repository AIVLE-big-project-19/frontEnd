...
test('구글 전용 계정(loginId가 null)이면 name을 식별자로 사용해 로그인 처리한다', async () => {
  authApi.googleLogin.mockResolvedValue({
    success: true,
    data: { accessToken: 'at', refreshToken: 'rt' },
  });
  myPageApi.getMyProfile.mockResolvedValue({
    loginId: null, name: 'z z', email: 'htmddus49@gmail.com', provider: 'GOOGLE',
  });
  ...
  await waitFor(() => expect(screen.getByText('메인페이지')).toBeInTheDocument());
  expect(mockAuthLogin).toHaveBeenCalledWith(
    { accessToken: 'at', refreshToken: 'rt' }, 'z z', true
  );
});
