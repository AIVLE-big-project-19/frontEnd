...
- **인증번호 카운트다운**: 5분(300초) — 백엔드 코드 유효시간과 일치(목업에는 3:00으로 표시되어 있으나 5분으로 확정). "인증번호 받기" 재클릭(재발송 성공) 시 타이머 재시작. 0이 되면 만료 안내를 표시하고 "인증번호 확인" 버튼 비활성화(재발송 필요).
- **입력값 변경 시 인증 상태 리셋**: 인증에 사용되는 입력값(아이디 찾기: 이메일 / 비밀번호 찾기: 아이디·이메일)을 인증 후에 변경하면 인증 완료 상태·발송 상태·타이머를 모두 리셋한다(SignupPage와 동일한 패턴). 백엔드 인증이 특정 값 조합에 묶여 있으므로 값이 바뀐 채 제출되는 것을 방지.
...
- FindIdPage: 인증 전/후 하단 버튼 활성화 상태, 카운트다운 만료 처리, 이메일 변경 시 인증 상태 리셋, `/show-id`로 state 전달 여부
- ShowIdPage: state 없이 접근 시 리다이렉트, 마스킹된 값 표시, 두 버튼의 이동 경로/state
- FindPasswordPage: 이메일 인증 전/후 "변경하기" 버튼 활성화, 아이디/이메일 변경 시 인증 상태 리셋, 비밀번호 조건/일치 검증, 완료 모달
